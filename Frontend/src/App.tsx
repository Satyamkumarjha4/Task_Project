...
export default function App(){
  ...
  useEffect(() => {
    const fetchTasks = async () => {
      if (visibleDates.length > 0) {
        const response = await axios.get(`${URL}/api/tasks/date/${visibleDates[0]}}`)
  ...
  const getUserTasks = (person: string, date: string) => {
    ...
    return tasks.filter((task) => task.person === person && task.date.substring(0, 11) === date)
  }
      ...
      <div className="flex flex-col md:flex-row gap-6 mb-8">
        ...
        <div className="flex-1">
          <h2 className="text-xl font-semibold mb-4">
            Viewing tasks from {format(new Date(visibleDates[0] || date), "MMM d, yyyy")} to{" "}
            {format(new Date(visibleDates[visibleDates.length - 1] || date), "MMM d, yyyy")}
          </h2>
          <p className="text-muted-foreground mb-2">
            Click on + to add a task. Click on status icons to cycle through: not started → in progress → completed.
          </p>
        </div>
      </div>
        ...
        <table className="w-full border-collapse">
          <thead>
            <tr className="bg-muted">
              <th className="border p-3 min-w-[150px] text-left">Team Member</th>
            ...
            {teamMembers.map((user) => (
              <tr key={user}>
                ...
                {visibleDates.map((date) => {
                  ...
                  return (
                    <td key={`${user}-${date}`} className="border p-3 align-top">
                      <div className="flex justify-between items-center mb-2">
                        <span className="text-xs text-muted-foreground">{userTasks.length} tasks</span>
                        <Button
                          //variant="ghost"
                          //size="sm"
                      ...
                      <div className="space-y-2">
                        {userTasks.map((task) => (
                          <div key={task.id} className="flex items-start gap-2 p-2 rounded bg-muted/50">
                            <button onClick={() => handleStatusChange(task)} className="mt-1 flex-shrink-0">
